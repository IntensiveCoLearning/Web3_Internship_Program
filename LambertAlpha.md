...
之前看了實習手冊的技術和訪談等其他部分，今晚又通讀了社區運營和法律合規部分，算是終於在最後一天把整個實習手冊學習完了，並且堅持了每一天的打卡。說實話，github的小勾勾讓人很有堅持下去的東西，惰性是有慣性的，但你一旦開始堅持，這種努力也是有慣性的。

今天算是一個小總結，這二十來天我也堅持每天都進步一天天的，累積下來學到了很多。一隻入門入了半年多的Foundry和Solidity，這次終於是真的入門了，如果不是這一點一滴積累的堅持，不知道又要等到猴年馬月才能入門了。

入門不等於學會所有東西，而是你大致知道了整體流程，能夠自己做一些小東西，在這做的過程中不斷髮現問題和去查看文檔解決，就算成功入門了。

實習計劃手冊也是，運營社區的指南，非談兵紙上，而是你自己要去辦一個AMA，然後學習文檔的流程。

我現在已經把技術、社區運營、BD的工作和要求都通讀學習了，也有去實踐落地的條件，沒必要再繼續空談學習，已經是到做的階段了。

所以，這是實習計劃的句號，是我通讀概覽學習的句號，也是真正實踐的開始。
...
1.  如果刚开始参加，比如参加sui的黑客松，可以去链上看sui上面热门的生态，他们在干啥，热点是什么？
    
2.  solo也没关系，全栈的话就没问题。技术栈：前端+合约，可以把核心功能放合约，业务和逻辑放前端。
    
3.  安排好实践很重要。大约三天把想法定好。做好规划：画一个原型；主要把时间精力放在代码上。三天左右准备文档，readme, ppt，这些是评委最初给你打分的依据。可以向大佬寻求建议。其实寻求建议。
...
比较长的黑客松目的是生态想接入一些项目进来。

比较长的黑客松更有利于想在生态里做点项目（相对长期的开发）
...
Presenting和前端很重要，评委会很看。

### 参加黑客松的收获（黑客松有什么作用呢？）

1.  做项目认识到队友，可以学习coding技巧，有人脉资源；
    
2.  获得了奖项，可以和项目方接触，让他们看到自己，对以后的求职有很好的影响，可以多了解呀，和生态项目方多聊聊，说不定可以融入生态；
...
2.  github去build，提pr，给生态做贡献；
    
3.  媒体：求职公众号和tg群；
    
4.  参与黑客松，短时间完成项目方的任务，展示技能；
    

cex,defi,dex开发者需求量是很大的

### 从招聘视角来看，开发岗需求量如何？

开发岗是需求量最大的岗位类型，占到50%

前端、测试、运维相对多一点，要求也会略低一点，薪资约低一点；

后端、全栈、智能合约，要求相对较高，薪资相对较高；

### 开发者的哪些技能更具有竞争力？

1.  核心技能，非常Global的一个行业，所以核心技能就是英文。
    
2.  不会像传统行业一样有培训，都是即招即用；
    
3.  自驱力，因为remote，所以自驱力很重要，不然很容易在熊市被优化掉。
    
4.  最好在应聘工作，有一份从0到1的一个项目经验，以及从1到99有什么优化经验？
...
今天下午面試Bybit的BI崗，被Python和SQL的基礎知識拷打了。大概花了一個小時做了一些復盤，總體來說就是對一些技術棧的基礎知識理解不夠深入。對於BI崗位，我的經歷其實不是很符合，沒有什麼重合度。

啟發就是，重要的面試有必要多花一些時間準備，大概留一週吧。把自己項目和jd對應的技術棧的八股都過一下吧，題也要多刷一些，其實就像面對學校的期末考，你能做到拿A的準備，那也大概率能夠成功通過了。
...
很簡單的一個軟件，基礎知識和用法看一下半個多小時的視頻就會了。

要點是：

### Dimensions和Measure是最重要的概念

以及還有Continuous和Discrete數據的區分，也可以相互轉換，區別是Continuous是具體的軸，Discrete是header

想起他圖表類型，用tableau做起來真的比python的**Matplotlib庫簡單好用太多，就是拖拖拖點點點就好了，也很美觀。**

藉由離散性和連續性的轉化來實現複雜時間的變換。

這個很簡單，具體的情境對應學就好了，沒必要再筆記了。

### Row Level Calc 和Aggregation區別

理解tableau這兩種計算方法的計算邏輯很重要

### 關係型數據庫和數據倉庫Hive的區別

| 特性 | MySQL / PostgreSQL (超市) | Hive (倉儲) |
| --- | --- | --- |
| 核心目的 | 線上交易處理 (OLTP)：處理大量、快速、簡短的「交易」。例如，用戶下單、註冊、登入。重點是「增刪改查」要快。 | 線上分析處理 (OLAP)：處理大量歷史數據的複雜「分析」。例如，分析過去一年所有用戶的交易行為模式。重點是「海量讀取和匯總」要穩。 |
| 數據模型 | 行式儲存 (Row-based Storage)：數據是按「行」儲存的。當你要UPDATE一筆訂單時，系統能快速找到那一行並修改。這非常適合交易場景。 | 列式儲存 (Column-based Storage)：數據是按「列」儲存的。當你要SUM(交易額)時，系統只需要讀取「交易額」那一列，而不用讀取整張表的其他列（如下單時間、用戶ID等），分析速度極快。 |
| 計算模型 | 單機或主從架構：計算主要集中在單一的伺服器上，雖然可以擴展，但處理 TB/PB 級數據會非常吃力。 | 分散式計算 (MapReduce/Tez/Spark)：基於 Hadoop 生態，將一個巨大的計算任務拆分成無數個小任務，交給一個由成百上千台機器組成的集群去並行處理，專為海量數據而生。 |
| 響應時間 | 毫秒級：你下單後必須立刻看到結果。 | 秒級到分鐘級：你提交一個分析查詢，可能需要跑幾十秒甚至幾分鐘，但它可以處理 MySQL 根本無法處理的數據量。 |
| 使用場景 | Bybit 的線上交易系統後端：支撐 App 和網站的即時讀寫操作。 | Bybit 的數據分析平台：儲存所有歷史交易紀錄、用戶行為日誌等，供 BI 團隊、數據科學團隊進行分析和報告。 |

### 其他一些工具

-   **Spark (及 PySpark)**：**強大的分散式計算引擎 (The Engine)**
    
    -   你可以把它看作是 Hive 使用的 MapReduce 引擎的「超級進化版」。它基於記憶體計算，速度比 MapReduce 快幾個數量級。
        
    -   **PySpark** 是 Spark 的 Python API，讓數據科學家和分析師可以用 Python 輕鬆地調用 Spark 的強大計算能力來處理海量數據。
        
    -   **使用場景**：用於數據管線中的 **T (Transform)** 階段。例如，用 PySpark 編寫一個複雜的 ETL 腳本，對 TB 級的原始交易日誌進行清洗、轉換和聚合，然後將結果存回資料倉儲。
        
-   **Presto (或 Trino)**：**高速的聯邦查詢引擎 (The Ad-hoc Query Tool)**
    
    -   它本身不儲存數據，它的超能力是**可以同時查詢儲存在不同地方的數據**。例如，你可以用一條 SQL，同時 join 儲存在 Hive 裡的數據、MySQL 裡的數據和另一種 NoSQL 資料庫裡的數據。
        
    -   它的查詢速度極快，非常適合 BI 分析師進行**即時的、探索性的 Ad-hoc 查詢**。
        
    -   **使用場景**：當儀表板需要快速響應，或者你需要立即回答一個突發的業務問題時，你可能會通過 Presto 來查詢數據，而不是等待 Hive 跑一個較慢的批次處理任務。
        
-   **Airflow**：**工作流調度與監控平台 (The Conductor)**
    
    -   它本身不做計算，它就像一個**數據管線的「總指揮」或「交響樂團的指揮家」**。
        
    -   你可以用 Python 程式碼定義一個複雜的工作流 (在 Airflow 中稱為 DAG)，例如：「每天凌晨 3 點，先執行 A 腳本從業務資料庫抽數據，成功後，並行執行 B 和 C 兩個 PySpark 腳本做數據轉換，等 B 和 C 都成功後，再執行 D 腳本把數據載入報表系統，並發送通知。」
        
    -   **使用場景**：自動化、管理和監控整個公司的數據管線，確保所有任務按時、按順序、可靠地執行。
        
-   **Flink**：**流處理計算引擎 (The Real-time Specialist)**
    
    -   如果說 Spark 更擅長**批次處理 (Batch Processing)**（處理一大批累積的數據，例如「計算昨天的總交易額」），那麼 Flink 則是**流處理 (Stream Processing)** 的王者。
        
    -   它能做到真正的**事件級別的即時計算**（數據一來就立刻處理）。
        
    -   **使用場景**：Bybit 的**即時風控系統**（偵測異常交易行為）、**即時數據大屏**（顯示交易所的即時交易量和用戶活動）、即時異常監控等。

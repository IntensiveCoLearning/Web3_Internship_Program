...
- **Farcaster** 是一个 **去中心化的社交协议**，运行在区块链与去中心化存储之上。
- 它的目标是成为一个 **开放、可组合（composable）、抗审查的社交网络基础设施**。
- 任何人都可以基于 Farcaster 协议去构建社交应用，类似于在 Web2 里有人在 Facebook API、Twitter API 上开发应用。
- **Farcaster = Web3 世界里的开放社交协议**，让用户拥有自己身份和数据，开发者可以基于它构建任何形式的社交应用。Warpcast 就是目前最热门的 Farcaster 客户端。
...
可以理解成这是 web3 上的去中心化的社交软件

</aside>

## 🔑 可以理解为

- **Farcaster 协议 = 数据层 & 通信规则（社交图谱、身份、消息）**
- **前端应用 = UI 层（Warpcast、第三方客户端）**

所以，Farcaster 提供 **社交协议 + API**，开发者用这些 API 去编写前端应用，但它本身 **不是前端开发协议**。
...
- 不依赖传统公司或管理层；
- 通过 **智能合约** 在区块链上运行；
- 由社区成员（通常持有代币的人）共同治理和决策。

---

## 🎮 什么是 BGA

**BGA = Blockchain Game Alliance**，中文叫 **区块链游戏联盟**。

- 它是一个 **国际性组织/联盟**，成员包括区块链项目方、游戏工作室、发行商、NFT 平台、投资人等。
- 成立的目标是 **推动区块链在游戏行业的应用**，促进标准化、合作和知识共享。

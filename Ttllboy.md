...
什么是函数修饰符？
函数修饰符是 Solidity 中用于修改函数行为的特殊语法，主要用于抽离重复逻辑（如权限校验、条件检查），让代码更简洁、可复用。
修饰符本身不实现核心功能，而是对被修饰的函数进行前置 / 后置增强（如先验证条件，再执行函数）。
核心作用
权限控制：限制只有特定地址（如合约 owner）能调用函数。
条件检查：确保函数执行前满足某些条件（如时间、余额限制）。
代码复用：将多个函数共有的校验逻辑抽离，避免重复编写。
什么是 Unix 时间戳？
Unix 时间戳是从 1970 年 1 月 1 日 00:00:00 UTC 开始计算的秒数，用于统一表示时间。
例如：1620000000 代表 2021 年 5 月 3 日 13:20:00 UTC。
Solidity 中获取时间戳
Solidity 通过全局变量 block.timestamp 获取当前区块的 Unix 时间戳（单位：秒），该值由矿工打包区块时确定（存在 ± 几秒误差）。
 什么是时间锁？
时间锁是一种延迟执行机制：用户提交操作后，需等待一段固定时间才能执行，用于提高合约安全性（如给用户留出反应时间，防止恶意操作）。
常见场景：重要参数修改、资金提取、合约升级等。
时间锁的核心逻辑
提交操作：记录操作内容和可执行的最早时间（当前时间 + 延迟时间）。
等待期：在这段时间内，操作不可执行（可取消）。
执行操作：等待期结束后，操作可被执行。
时间锁的关键设计
延迟时间：根据业务风险设置（如重要操作延迟 7 天，普通操作延迟 12 小时）。
过期机制：设置操作有效期（如延迟结束后 24 小时内未执行则失效），避免僵尸操作。
权限控制：通常限制只有特定角色（如治理合约）能提交操作，普通用户可查看和监督。

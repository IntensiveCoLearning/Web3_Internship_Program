...
Git提交到远程仓库：推送的是整个提交历史，而不仅仅是最后一次提交。

Chainlink 是一个去中心化的预言机网络（Decentralized Oracle Network）。它的核心作用是为区块链上的智能合约提供可靠、真实的外部世界数据

动态交易费+chainlink预言机：

用平滑函数（如Sigmoid函数变体）来防止费率在边界处突变，使调整更加平滑自然。

省gas+避免改费率过于频繁影响用户体验每小时更新一次费率

Pair的swap并不计算花多少手续费，仅假设扣除0.3%手续费流动性是否减少。手续费在library计算。所以在library计算时要拿到每个代币对的波动率算出的手续费率，然后算
<!-- DAILY_CHECKIN_2025-08-27_END -->


# 2025-08-25
<!-- DAILY_CHECKIN_2025-08-25_START -->
学习chainlink预言机，预言机的作用，了解并选择添加动态交易费功能作为个人学习项目的丰富方向，了解套利及恶意套利等方式

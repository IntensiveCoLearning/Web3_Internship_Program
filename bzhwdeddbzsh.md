...
DApp 开发通常包括前端、智能合约、数据检索器以及区块链和去中心化存储四个部分，前端负责与用户交互并连接钱包调用合约，智能合约用 Solidity 编写并在区块链上执行核心业务逻辑，检索器将链上事件数据处理后存入数据库以供前端使用。开发流程涵盖需求分析、合约编写与审计、检索器与前端开发、链上交互以及最终部署上线，贯穿始终的目标是确保功能完整、安全可靠，并能高效展示和操作链上数据。
...
- 前端：与传统 Web 应用不同，Dapp 前端会连接区块链来调用智能合约，呈现数据和执行交易。以及需要在这里来链接钱包，主要要学习React。
- 智能合约：智能合约是 Dapp 的核心，它定义了应用的业务逻辑，并部署在区块链上。一般用Solidity写，在EVM上执行。
- 数据检索：智能合约通常以 `Event` 形式释放日志事件，比如释放代表 NFT 转的 `Transfer` 事件，数据检索器会把他们储存到传统数据库中。**这一部分实现了链上数据链接到链下，用于和DApp的前端进行交互。**
- 区块链和去中心化存储：确保所有数据在多个节点上备份，保证数据的持久性和去中心化特性。

## 开发流程

这部分文档已经讲的很清楚了，就直接copy，会着重加粗之前没有考虑过的问题
...
2. **智能合约开发**
    ...
    智能合约是 Dapp 的核心，负责执行去中心化的业务逻辑和存储重要的数据。
...
3. **检索器开发**
    ...
    检索器是获取链上数据的核心，负责捕获智能合约释放的事件并以合理的方式将其存入数据库的不同的表内部。
...
4. **前端开发**
    ...
    - 选择前端框架：一般用React来构建 UI。前端将通过 JavaScript 与智能合约进行交互。
...
阅读部分看完了，接下来就是环境配置以及学习语法的实操环节，今天实在是太忙了，就先到此为止，剩下的明天继续
